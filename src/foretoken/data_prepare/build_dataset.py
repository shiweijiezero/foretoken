...
内容决定、配置无关,不复刻 Mooncake 的 512 块量化)。
"""
...
if __name__ == "__main__":  # pragma: no cover
    ...
    ap.add_argument(
        "--mooncake-config",
        default="conversation,mooncake,toolagent",
    ...
    for cfg in (c.strip() for c in args.mooncake_config.split(",") if c.strip()):
        sess_c = reconstruct_sessions(
            list(_stream_hf(args.mooncake, "train", args.limit, config=cfg))
        )
        rows_c = list(
            fill_sessions(sess_c, _stream_hf(args.content, "train", None), seed=args.seed)
        )
        splits[cfg] = rows_c
