...
fn plan() -> LaunchPlanV1 {
    LaunchPlanV1::parse(r#"{"version":1,"artifacts":{"model":"model","revision":"rev","tokenizer":"tokenizer","tokenizerRevision":"tokenizer-rev"},"parallelism":{"tp":2,"pp":1,"dp":1,"pcp":1,"dcp":1},"kv":{"kind":"none","events":true},"lifecycle":{"startupSeconds":30,"drainSeconds":7},"extraArgs":["--max-model-len=32768"]}"#).unwrap()
}

#[test]
fn rejects_unknown_and_missing_wire_fields() {
    assert!(LaunchPlanV1::parse(r#"{"version":1,"artifacts":{},"parallelism":{},"kv":{"kind":"none","events":true},"lifecycle":{},"extraArgs":[],"unexpected":true}"#).is_err());
...
fn ec_plan_renders_one_owned_config_and_reports_a_role_independent_fingerprint() {
    let producer = LaunchPlanV1::parse(r#"{"version":1,"artifacts":{"model":"m","revision":"r","tokenizer":"t","tokenizerRevision":"tr"},"parallelism":{"tp":1,"pp":1,"dp":1,"pcp":1,"dcp":1},"kv":{"kind":"none","events":true},"ec":{"profileName":"verified-ec","profileRevision":"r1","connector":"ECExampleConnector","role":"producer","runtimeFingerprint":"pinned-r1","sharedStoragePath":"/var/lib/foretoken/ec"},"lifecycle":{"startupSeconds":1,"drainSeconds":1},"extraArgs":[]}"#).unwrap();
    let consumer = LaunchPlanV1::parse(r#"{"version":1,"artifacts":{"model":"m","revision":"r","tokenizer":"t","tokenizerRevision":"tr"},"parallelism":{"tp":1,"pp":1,"dp":1,"pcp":1,"dcp":1},"kv":{"kind":"none","events":true},"ec":{"profileName":"verified-ec","profileRevision":"r1","connector":"ECExampleConnector","role":"consumer","runtimeFingerprint":"pinned-r1","sharedStoragePath":"/var/lib/foretoken/ec"},"lifecycle":{"startupSeconds":1,"drainSeconds":1},"extraArgs":[]}"#).unwrap();
...
fn kv_variants_render_expected_semantics() {
    let cases = [
        ...
        (
            r#"{"kind":"multiConnector","role":"kv_producer","protocol":"rdma","events":true}"#,
            "MultiConnector",
        ),
    ];
    for (kv, want) in cases {
        let source = format!(
            r#"{{"version":1,"artifacts":{{"model":"m","revision":"r","tokenizer":"t","tokenizerRevision":"tr"}},"parallelism":{{"tp":1,"pp":1,"dp":1,"pcp":1,"dcp":1}},"kv":{kv},"lifecycle":{{"startupSeconds":1,"drainSeconds":1}},"extraArgs":[]}}"#
        );
