...
`foretoken-frontend` 是位于平台托管、兼容 Gateway API 的 Gateway 之后、直接处理推理请求的 Rust 数据面。它尽可能复用 vLLM Rust 已有的请求处理器和增量输出 API，由 Foretoken 补充多模型路由、后端选择与 P/D 编排能力。
...
data-plane/frontend/src/router/
  按 model、revision、capability 和 health 构造候选集，再按 KV locality 和 load 选择后端
        ↓
data-plane/frontend/src/backend-registry/
...
Aggregate 请求只使用一个后端。P/D 请求先选择并执行一个 Prefill 节点，完成后重新读取最新 backend 状态，再选择一个 Decode 节点。P 和 D 不配对，也不同时预留；Frontend 只把 Decode stream 返回给调用方，stream 被提前丢弃时会中止尚未完成的后端任务。

## Crate 职责与组合

`cmd` 是整个 Frontend 的 composition root。它负责读取和校验 serving snapshot，构建 `BackendRegistry`、`KvIndexer` 和每个模型的 processor，最后把它们组合成 `PipelineRouter` 与完整的 `RuntimeState`。
...
data-plane/frontend/src/router/            后端节点过滤、评分与分阶段选择
...
`kv-indexer` 定义并实现 KV prefix 查询接口，Router 只消费查询结果，不关心索引如何构建；两者由 Frontend composition root 组合。
...
Foretoken 将 Frontend 打包为 OCI image。由于 Rust workspace 直接复用 `third_party/vllm-rust/source/rust` 中的本地 vLLM 构建源码，镜像需要从仓库根目录构建。

控制面负责创建和管理 Frontend Deployment，挂载 serving snapshot 与 tokenizer cache，并通过平台托管、兼容 Gateway API 的 Gateway 对外提供服务。用户不需要单独创建或启动 Frontend 副本。

## 开发构建

使用本地 vLLM Git checkout 作为构建源码：
...
源码选择器会验证 checkout，并更新 Git 忽略的 `third_party/vllm-rust/source` 链接；它不会修改该 checkout。
