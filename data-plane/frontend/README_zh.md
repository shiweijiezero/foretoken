...
通过 `ModelService` 配置聚合部署、预填充/解码分离（P/D）或编码/预填充/解码分离（E/P/D）。分离式推理需要平台支持所选运行时和传输方式，前端请求参数不会改变部署拓扑。
...
`/v1/models` 返回前端当前生效配置中的模型。模型出现在列表中不保证后续请求时其后端仍然健康；客户端需要处理不可用响应。
...
模型和路由更新只会在准备成功后生效。无效或未就绪的更新不会替换当前配置，已经开始执行的请求继续使用开始时的配置。
