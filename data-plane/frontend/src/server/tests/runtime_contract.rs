...
fn published_state(model: &str, revision: &str) -> Arc<RuntimeState> {
    Arc::new(RuntimeState::new(
        BTreeMap::from([(model.into(), test_runtime(revision))]),
