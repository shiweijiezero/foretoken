...
//! Read-only route target statistics available to routing algorithms.
...
pub struct RouteTargetStats {
    ...
    /// Actual interval between the selected baseline and latest snapshot.
...
pub trait RouteTargetStatsReader: Send + Sync {
    /// Calculates statistics for `candidate` over the requested `window`.
    ///
