...
- `allow_all`：默认直通过滤器。保留所有健康且满足基本兼容条件的候选项进入评分阶段，不进行额外过滤。

**Scorers:**

- `kv_least_loaded`：KV cache 本地性和低负载优先策略。评估维度按优先级依次为：
...
- `max`：选择得分最高的节点。出现同分时，以 `route_target_id` 最小的节点作为确定性的破同分规则。
