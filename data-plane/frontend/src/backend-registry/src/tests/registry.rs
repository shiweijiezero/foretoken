use super::*;
use axum::Json;
use axum::Router;
use axum::http::StatusCode;
use axum::routing::get;
use foretoken_model_protocol::{
    RuntimeEcTransferMetadata, RuntimeMetadataResponse, RuntimeModelIdentity, TelemetryResponse,
    VLLM_PINNED_REVISION,
};
use tokio::net::TcpListener;
use vllm_engine_core_client::protocol::dtype::ModelDtype;

use crate::{
    BackendRegistryBuild, SnapshotEpdComponent, SnapshotEpdDomain, SnapshotGroup,
    SnapshotPdComponent, SnapshotPdDomain,
};
use foretoken_router::{BackendRole, ExecutionPlan};

fn component(id: &str, role: BackendRole) -> SnapshotPdComponent {
    SnapshotPdComponent {
        backend_id: BackendId::new(id),
        role,
        domain_id: "service-a".into(),
        model: "model".into(),
        revision: "r1".into(),
        tokenizer: "tokenizer".into(),
        tokenizer_revision: "r1".into(),
        profile_name: "profile".into(),
        profile_revision: "r1".into(),
        connector: "MooncakeConnector".into(),
        protocol: "rdma".into(),
        capabilities: ["chat".into()].into_iter().collect(),
        max_input_tokens: None,
        endpoint: "http://127.0.0.1:9000".into(),
        prefill_bootstrap_endpoint: (role == BackendRole::Prefill)
            .then(|| "http://127.0.0.1:29001".into()),
        kv_scope_id: "scope".into(),
    }
}
fn pd_snapshot() -> ServingSnapshot {
    ServingSnapshot {
        version: 1,
        groups: vec![],
        pd_components: vec![
            component("p", BackendRole::Prefill),
            component("d", BackendRole::Decode),
        ],
        pd_domains: vec![SnapshotPdDomain {
            domain_id: "service-a".into(),
            prefill_backend_ids: vec![BackendId::new("p")],
            decode_backend_ids: vec![BackendId::new("d")],
        }],
        epd_components: vec![],
        epd_domains: vec![],
    }
}

fn epd_component(id: &str, role: BackendRole) -> SnapshotEpdComponent {
    let pd = role != BackendRole::Encoder;
    let ec = role != BackendRole::Decode;
    SnapshotEpdComponent {
        backend_id: BackendId::new(id),
        role,
        domain_id: "epd-a".into(),
        model: "model".into(),
        revision: "r1".into(),
        tokenizer: "tokenizer".into(),
        tokenizer_revision: "r1".into(),
        profile_name: if pd {
            "pd-profile".into()
        } else {
            String::new()
        },
        profile_revision: if pd { "r1".into() } else { String::new() },
        connector: if pd {
            "MooncakeConnector".into()
        } else {
            String::new()
        },
        protocol: if pd { "rdma".into() } else { String::new() },
        ec_profile_name: if ec {
            "ec-profile".into()
        } else {
            String::new()
        },
        ec_profile_revision: if ec { "r1".into() } else { String::new() },
        ec_connector: if ec {
            "ECExampleConnector".into()
        } else {
            String::new()
        },
        ec_runtime_fingerprint: if ec {
            "fingerprint".into()
        } else {
            String::new()
        },
        capabilities: ["chat".into()].into_iter().collect(),
        max_input_tokens: None,
        endpoint: "http://127.0.0.1:9000".into(),
        prefill_bootstrap_endpoint: (role == BackendRole::Prefill)
            .then(|| "http://127.0.0.1:29001".into()),
        kv_scope_id: "scope".into(),
    }
}
fn epd_snapshot() -> ServingSnapshot {
    ServingSnapshot {
        version: 1,
        groups: vec![],
        pd_components: vec![],
        pd_domains: vec![],
        epd_components: vec![
            epd_component("e", BackendRole::Encoder),
            epd_component("p", BackendRole::Prefill),
            epd_component("d", BackendRole::Decode),
        ],
        epd_domains: vec![SnapshotEpdDomain {
            domain_id: "epd-a".into(),
            encoder_backend_id: BackendId::new("e"),
            prefill_backend_id: BackendId::new("p"),
            decode_backend_id: BackendId::new("d"),
        }],
    }
}

fn runtime_metadata(model: &str, revision: &str) -> RuntimeMetadataResponse {
    RuntimeMetadataResponse {
        version: 1,
        model: RuntimeModelIdentity {
            model: model.into(),
            revision: revision.into(),
        },
        model_dtype: ModelDtype::BFloat16,
        effective_max_model_len: 32_768,
        vllm_pinned_revision: VLLM_PINNED_REVISION.into(),
        vllm_version: "0.0.0".into(),
        ec_transfer: None,
        capabilities: ["chat".into()].into_iter().collect(),
    }
}

async fn serve_model_server(metadata: RuntimeMetadataResponse) -> String {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let endpoint = format!("http://{}", listener.local_addr().unwrap());
    let metadata_route = metadata.clone();
    let app = Router::new()
        .route("/readyz", get(|| async { StatusCode::OK }))
        .route(
            "/v1/internal/metadata",
            get(move || {
                let metadata = metadata_route.clone();
                async move { Json(metadata) }
            }),
        )
        .route(
            "/v1/internal/telemetry",
            get(|| async {
                Json(TelemetryResponse {
                    version: 1,
                    accepting: true,
                    running_requests: 0,
                    max_concurrent_requests: 1,
                })
            }),
        );
    tokio::spawn(async move { axum::serve(listener, app).await.unwrap() });
    endpoint
}
fn aggregate_snapshot(endpoint: String) -> ServingSnapshot {
    ServingSnapshot {
        version: 1,
        groups: vec![SnapshotGroup {
            backend_id: BackendId::new("a"),
            model: "model".into(),
            revision: "r1".into(),
            tokenizer: "tokenizer".into(),
            tokenizer_revision: "r1".into(),
            capabilities: ["chat".into(), "multimodal".into()].into_iter().collect(),
            max_input_tokens: None,
            endpoint,
            kv_scope_id: "scope".into(),
        }],
        pd_components: vec![],
        pd_domains: vec![],
        epd_components: vec![],
        epd_domains: vec![],
    }
}

#[tokio::test]
async fn readiness_preserves_frontend_owned_capabilities() {
    let endpoint = serve_model_server(runtime_metadata("model", "r1")).await;
    let registry = BackendRegistry::from_snapshot(aggregate_snapshot(endpoint)).unwrap();

    registry.refresh_backend_readiness().await;

    assert!(registry.is_backend_healthy(&BackendId::new("a")));
    assert_eq!(
        registry.metadata(&BackendId::new("a")),
        Some(runtime_metadata("model", "r1"))
    );
    assert_eq!(
        registry.effective_capabilities(&BackendId::new("a")),
        ["chat".into(), "multimodal".into()].into_iter().collect()
    );
    assert_eq!(registry.effective_max_model_len("model"), Some(32_768));
    assert_eq!(
        registry.effective_model_dtype("model"),
        Some(ModelDtype::BFloat16)
    );
}

#[tokio::test]
async fn readiness_rejects_metadata_with_a_different_snapshot_identity() {
    let endpoint = serve_model_server(runtime_metadata("other-model", "r1")).await;
    let registry = BackendRegistry::from_snapshot(aggregate_snapshot(endpoint)).unwrap();

    registry.refresh_backend_readiness().await;

    assert!(!registry.is_backend_healthy(&BackendId::new("a")));
    assert!(registry.metadata(&BackendId::new("a")).is_none());
}

#[tokio::test]
async fn readiness_rejects_metadata_with_a_different_pinned_revision() {
    let mut metadata = runtime_metadata("model", "r1");
    metadata.vllm_pinned_revision = "different-pin".into();
    let endpoint = serve_model_server(metadata).await;
    let registry = BackendRegistry::from_snapshot(aggregate_snapshot(endpoint)).unwrap();

    registry.refresh_backend_readiness().await;

    assert!(!registry.is_backend_healthy(&BackendId::new("a")));
    assert!(registry.metadata(&BackendId::new("a")).is_none());
}

#[test]
fn epd_runtime_metadata_must_match_the_controller_owned_ec_contract() {
    let expected = RuntimeExpectation {
        model: "model".into(),
        revision: "r1".into(),
        ec_transfer: Some(RuntimeEcTransferMetadata {
            role: "ec_producer".into(),
            profile: "ec-profile".into(),
            connector: "ECExampleConnector".into(),
            fingerprint: "fingerprint".into(),
        }),
    };
    let mut metadata = runtime_metadata("model", "r1");
    metadata.ec_transfer = expected.ec_transfer.clone();
    assert!(metadata_matches(&expected, &metadata));

    metadata.ec_transfer.as_mut().unwrap().fingerprint = "other".into();
    assert!(!metadata_matches(&expected, &metadata));
}

#[test]
fn pd_metadata_dtype_or_context_mismatch_disables_the_entire_pair_domain() {
    let registry = BackendRegistry::from_snapshot(pd_snapshot()).unwrap();
    let mut prefill = runtime_metadata("model", "r1");
    prefill.model_dtype = ModelDtype::Float16;
    let decode = runtime_metadata("model", "r1");
    let mut results = vec![
        (
            BackendId::new("p"),
            true,
            Some(prefill),
            Some(foretoken_router::ComponentCapacity {
                component_id: BackendId::new("p"),
                running_requests: 0,
                max_concurrent_requests: 1,
            }),
        ),
        (
            BackendId::new("d"),
            true,
            Some(decode),
            Some(foretoken_router::ComponentCapacity {
                component_id: BackendId::new("d"),
                running_requests: 0,
                max_concurrent_requests: 1,
            }),
        ),
    ];

    invalidate_incompatible_pd_pairs(registry.route_table(), &mut results);

    assert!(!results[0].1);
    assert!(!results[1].1);
}

#[test]
fn epd_metadata_dtype_or_context_mismatch_disables_the_entire_triplet() {
    let registry = BackendRegistry::from_snapshot(epd_snapshot()).unwrap();
    let encoder = runtime_metadata("model", "r1");
    let prefill = runtime_metadata("model", "r1");
    let mut decode = runtime_metadata("model", "r1");
    decode.effective_max_model_len = 16_384;
    let mut results = [
        (BackendId::new("e"), true, Some(encoder), None),
        (BackendId::new("p"), true, Some(prefill), None),
        (BackendId::new("d"), true, Some(decode), None),
    ];

    invalidate_incompatible_pd_pairs(registry.route_table(), &mut results);

    assert!(results.iter().all(|(_, healthy, _, _)| !healthy));
}

#[test]
fn builds_component_inventory_without_cartesian_links() {
    let registry = BackendRegistry::from_snapshot(pd_snapshot()).unwrap();
    assert_eq!(registry.route_table().routes().len(), 2);
    assert!(
        registry
            .route_table()
            .routes()
            .iter()
            .any(|route| route.role == BackendRole::Prefill)
    );
    assert!(
        registry
            .route_table()
            .routes()
            .iter()
            .any(|route| route.role == BackendRole::Decode)
    );
}

#[test]
fn preserves_pd_input_limits_in_route_inventory() {
    let mut snapshot = pd_snapshot();
    snapshot.pd_components[0].max_input_tokens = Some(4096);
    snapshot.pd_components[1].max_input_tokens = Some(8192);

    let registry = BackendRegistry::from_snapshot(snapshot).unwrap();
    let routes = registry.route_table().routes();

    assert_eq!(
        routes
            .iter()
            .find(|route| route.role == BackendRole::Prefill)
            .unwrap()
            .max_input_tokens,
        Some(4096)
    );
    assert_eq!(
        routes
            .iter()
            .find(|route| route.role == BackendRole::Decode)
            .unwrap()
            .max_input_tokens,
        Some(8192)
    );
}

#[test]
fn rejects_component_not_in_its_service_domain() {
    let mut snapshot = pd_snapshot();
    snapshot.pd_domains[0].decode_backend_ids.clear();
    assert!(matches!(
        BackendRegistry::from_snapshot(snapshot),
        Err(SnapshotError::InvalidPdDomain(_))
    ));
}
#[test]
fn resolver_constructs_one_dynamic_pd_facade() {
    let registry = BackendRegistry::from_snapshot(pd_snapshot()).unwrap();
    let plan = ExecutionPlan::PrefillDecode {
        prefill: foretoken_router::RouteDecision {
            backend_id: BackendId::new("p"),
            model: "model".into(),
            revision: "r1".into(),
        },
        decode: foretoken_router::RouteDecision {
            backend_id: BackendId::new("d"),
            model: "model".into(),
            revision: "r1".into(),
        },
    };
    assert!(
        <BackendRegistry as foretoken_llm_facade::LlmFacadeResolver>::resolve(&registry, &plan)
            .is_some()
    );
}
#[test]
fn resolver_constructs_one_dynamic_epd_facade() {
    let registry = BackendRegistry::from_snapshot(epd_snapshot()).unwrap();
    let decision = |id| foretoken_router::RouteDecision {
        backend_id: BackendId::new(id),
        model: "model".into(),
        revision: "r1".into(),
    };
    let plan = ExecutionPlan::EncoderPrefillDecode {
        encoder: decision("e"),
        prefill: decision("p"),
        decode: decision("d"),
    };
    assert!(
        <BackendRegistry as foretoken_llm_facade::LlmFacadeResolver>::resolve(&registry, &plan)
            .is_some()
    );
}

#[test]
fn readiness_requires_a_healthy_pd_pair() {
    let registry = BackendRegistry::from_snapshot(pd_snapshot()).unwrap();
    registry.health[&BackendId::new("p")].store(true, Ordering::Release);
    assert!(!registry.is_ready());
    assert!(registry.healthy_models().is_empty());

    registry.health[&BackendId::new("d")].store(true, Ordering::Release);
    assert!(registry.is_ready());
    assert_eq!(registry.healthy_models(), vec!["model"]);
}

#[test]
fn readiness_requires_all_healthy_epd_stages() {
    let registry = BackendRegistry::from_snapshot(epd_snapshot()).unwrap();
    for id in ["p", "d"] {
        registry.health[&BackendId::new(id)].store(true, Ordering::Release);
    }
    assert!(!registry.is_ready());
    assert!(registry.healthy_models().is_empty());

    registry.health[&BackendId::new("e")].store(true, Ordering::Release);
    assert!(registry.is_ready());
    assert_eq!(registry.healthy_models(), vec!["model"]);
}

#[test]
fn epd_snapshot_requires_one_compatible_static_triplet() {
    let registry = BackendRegistry::from_snapshot(epd_snapshot()).unwrap();
    assert_eq!(registry.route_table().routes().len(), 3);
    assert!(
        registry
            .route_table()
            .routes()
            .iter()
            .any(|route| route.role == BackendRole::Encoder)
    );

    let mut incompatible = epd_snapshot();
    incompatible.epd_components[1].ec_runtime_fingerprint = "other".into();
    assert!(matches!(
        BackendRegistry::from_snapshot(incompatible),
        Err(SnapshotError::InvalidEpdDomain(_))
    ));

    let mut not_a_triplet = epd_snapshot();
    not_a_triplet.epd_domains[0].decode_backend_id = BackendId::new("other");
    assert!(matches!(
        BackendRegistry::from_snapshot(not_a_triplet),
        Err(SnapshotError::InvalidEpdDomain(_))
    ));
}

#[test]
fn epd_components_are_excluded_from_the_kv_prefix_index() {
    let build = BackendRegistryBuild::from_snapshot(epd_snapshot()).unwrap();
    assert!(build.kv_runtime_config.route_bindings.is_empty());
    assert!(build.kv_runtime_config.event_sources.is_empty());
}

#[test]
fn kv_index_binds_and_polls_only_prefill_components() {
    let build = BackendRegistryBuild::from_snapshot(pd_snapshot()).unwrap();
    assert_eq!(build.kv_runtime_config.route_bindings.len(), 1);
    assert_eq!(build.kv_runtime_config.event_sources.len(), 1);
    assert!(
        build
            .kv_runtime_config
            .route_bindings
            .contains_key(&BackendId::new("p"))
    );
    assert_eq!(
        build.kv_runtime_config.event_sources[0].backend_id,
        BackendId::new("p")
    );
}
