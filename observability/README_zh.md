...
在 Grafana 中打开 Foretoken 系统概览，或英文版 Foretoken System Overview。选择命名空间和模型，即可查看模型整体的输入、输出吞吐量、完成请求、调度负载、延迟、缓存命中率及 CPU／内存用量。每个模型分别展示总量和时间趋势。

按模型实例、执行角色或引擎 rank 筛选后端明细时，模型总计保持不变。实例名称由服务名、Pool（执行池）和副本序号组成；替换期间出现同名实例时，用创建时间区分。“路由决策”展示同一模型、同一角色内各后端的占比；一个后端对应一个模型实例和数据并行 rank，筛选后分母仍保留该模型该角色的全部后端。
...
选择功耗告警时，还需按显卡型号填写正数 `spec.observability.alerts.thresholds.nvidiaPowerWatts`，单位为瓦。只填写阈值不会启用规则。通知接入见 [Lark](integrations/lark/README_zh.md)、[Slack](integrations/slack/README_zh.md) 或 [钉钉](integrations/dingtalk/README_zh.md) 集成。
