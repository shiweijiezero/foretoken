...
The route selects `service=foretoken`, the common label installed by Foretoken's alert rules. The payload groups alerts by alert name, lists each affected target, reports firing and resolved timestamps in `Asia/Shanghai`, and includes `summary`, `description`, and `runbook_url` from each alert rule.
