...
平台统一在数据挂载下派生模型根目录，分别投射模型目录、来源缓存和编译缓存的位置。artifacts 库统一负责数据面两端的本地模型和 tokenizer 目录解析。相对引用限制在模型根目录内，单个文件会明确报错，不会被替换成父目录。库不下载模型，也不校验模型格式；这些职责属于推理引擎与 tokenizer loader。本地文件解析后，公开模型标识保持不变。
...
Triton JIT 产物使用 Pod 本地 `/tmp` 卷，因为并发发布缓存需要本地文件系统支持原子重命名，包括替换已加载的 launcher。该缓存随 Pod 回收。
