...
The persistent data root contains `models`, `vllm`, `torch`, `triton`, and `profiles`. Prepared models resolve under `models`; Hub repositories retain the upstream `models/hub` layout with revision-aware cache lookup. Bounded diagnostic captures publish below `profiles/runs`. Temporary startup retries relocate only projected provider/compiler cache paths and do not accept profile output, while prepared local models continue resolving from the persistent model root. No namespace or Pod identity is added to persistent model paths.
