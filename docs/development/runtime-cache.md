...
The model-files library owns local model/tokenizer directory resolution for both data-plane consumers. It keeps relative references inside the mounted root and rejects individual files rather than replacing them with parent directories. It does not download artifacts or validate model formats; those responsibilities remain with the inference engine and tokenizer loaders. Public model identifiers remain unchanged after resolving local files.
