...
Triton JIT artifacts instead use Pod-local `/tmp` storage and are discarded when the Pod is removed.

The persistent data root contains `models`, `vllm`, `torch`, and `profiles`. Prepared models resolve under `models`; Hub repositories retain the upstream `models/hub` layout with revision-aware cache lookup. Diagnostic captures publish below `profiles/runs` and are unavailable during a temporary-cache retry. Prepared local models continue resolving from the persistent model root. No namespace or Pod identity is added to persistent model paths.
