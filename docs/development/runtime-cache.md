...
The runtime cache mount stores model-provider caches and engine compilation caches such as vLLM and TorchInductor. For single-node services, model-server can stop a failed EngineCore and retry under the Pod's `/tmp` volume when the persistent cache fails its write probe. Multi-node members restart together instead of retrying independently. A frontend with a missing Hub snapshot can use its temporary tokenizer cache. This fallback neither changes a running engine's storage path nor copies prepared local checkpoints.

Triton's initial cache uses `triton/<node-name>` on the PVC, separating node writes while retaining reuse after Pod replacement. Cache location changes follow the Group rollout; existing Groups keep their original environment until replaced. Engine-managed compiler subdirectories remain under vLLM's control.

The persistent data root contains `models`, `vllm`, `torch`, `triton`, and `profiles`. Prepared models resolve under `models`; Hub repositories retain the upstream `models/hub` layout with revision-aware cache lookup. Diagnostic captures publish below `profiles/runs` and are unavailable during a temporary-cache retry. Prepared local models continue resolving from the persistent model root. No namespace or Pod identity is added to persistent model paths.
