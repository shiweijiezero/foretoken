...
- 实现:进程内自起 vLLM 引擎(`AsyncLLM`),无独立 HTTP server、进程退出即释放 GPU。`config/models/<model>.toml`(每模型一文件,`[sampling]` + `[serve]`)→ `params_for` → replay 按 `--model`(权重路径)/`--name`(逻辑名)自动取;采样常用项 `--temperature/--top-p/--top-k/--max-tokens/--seed` 或 `--param K=V`、引擎字段 `--engine-param K=V`(任意 `AsyncEngineArgs`,如 `kv_cache_dtype=fp8`/`cpu_offload_gb=20`)可命令行覆盖。
