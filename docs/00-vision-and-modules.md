...
## 愿景
**大型 Dense/MoE 模型的工业级部署 + 推理**,多节点、高并发——**这是主体**。我们不重造已经
成熟的各层 —— 而是**在 vLLM 之上组装业界最优的现有模块,只自建有差异化价值的那部分**,并用
**真正 real 的评测**检验每一项优化的好坏(只有 real 的评测才知道好坏,见 `docs/04`/`docs/07`)。
差异化的优化是一组杠杆:**价值感知的长生命周期 KV-cache 管理**(最大、最难的一块)、**MTP
投机解码**,由 goodput 感知的控制环路把它们串联起来,统一标尺是每 GPU 字节秒的 goodput。

**与市场对齐**(`docs/01`):引擎层 + 控制平面都已拥挤/成熟 → 复用它们。尚未攻下的缺口
(KV 价值策略、goodput 最优的投机控制)→ 自建。这张地图把 build-vs-use 判定逐模块地明确
下来,让每个模块都得到定向研究,我们绝不重造已经存在的东西。

## 图例
**Use** = 原样采用现有工具 · **Extend** = 配置/接入现有接口面(不 fork) · **Build** = 我们的
差异化代码 · **Fork** = 修改引擎核心(最后手段)。状态:✅ 已调研 · 🔬 需定向研究 · ⬜ 未开始。
Phase = P0–P4(`DESIGN.md §6`)。
...
## B. KV-cache 管理 —— 我们的差异化核心(`DESIGN.md`、`docs/02/03`)
| # | 模块 | 判定 | 工具 / 做法 | 状态 | phase |
|---|---|---|---|---|---|
...
| B8 | `P(reuse)` 长视野估计器(最难的部分) | **Build** | 按类生存 + 不确定性 + 竞争安全混合 | 🔬(最难) | P1 |

## C. 投机解码 —— **MTP 是一个关键模块**(用户的着重点)
| # | 模块 | 判定 | 工具 / 做法 | 状态 | phase |
|---|---|---|---|---|---|
| **C1** | **MTP 投机解码**(原生多 token 预测头,如 GLM-4.5 / DeepSeek MTP) | **Use→Extend** | vLLM `glm4_moe_mtp` / deepseek-MTP spec 路径;调 draft 长度,与我们的 scheduler/KV 集成 | 🔬 | P2/3 |
| C2 | goodput 感知投机控制(自适应 draft 长度、SLO 感知) | **Build/research** | SmartSpec/SpecServe/AdaServe 一类 —— *OSS 中未产品化*;我们的控制环路 | 🔬 | P3 |
...
| # | 模块 | 判定 | 工具 / 做法 | 状态 | phase |
|---|---|---|---|---|---|
| F1 | 指标(TTFT/TPOT/goodput/KV 命中/缓存) | **Extend** | vLLM Prometheus + 我们的 KV/goodput 指标 | 🔬 | P0 |
...
| # | 模块 | 判定 | 工具 / 做法 | 状态 | phase |
|---|---|---|---|---|---|
| G1 | trace 回放(Mooncake)+ 长生命周期负载 | **Build** | vLLM `bench serve --dataset-name timed_trace` + 我们的长生命周期构造器 | ✅ 设计 | P0 |
| G2 | 基线 + 指标 + PFOO oracle + 门槛零 | **Build** | 原生 LRU / T-LRU / SAECache / LMCache;每字节 goodput;回放正确性门槛 | ✅ 设计 | P0 |

## 我们自建什么(差异化集合)vs 复用什么
- **自建(我们的 IP):** B3 价值感知生命周期(MVP)· B5 复用 vs 重算 · B8 `P(reuse)` 估计器 ·
  B4 非前缀复用 · D4 goodput 调度 · C2 goodput 感知投机控制 · G1/G2 评测测试台 · F3 追踪。
- **复用/扩展(成熟):** 整个引擎(A*)、KV 机制(B1/B2/B6/B7)、MTP(C1)、批处理/P-D
  (D1/D3)、整个控制平面 + 部署(E*)、基础指标/确定性(F1/F2)。
- **推迟:** C3 跨词表;E2–E4 集群;F3 追踪 —— 在差异化核心被验证之后。

## 跨模块约束(调研中发现 —— 必须遵守)
- **MTP draft KV 是临时的**(`docs/05`):drafter 的 `kv_cache_gid` 为被拒绝的 draft 写入的
  KV 会被丢弃 → **B2/B3 必须把 draft KV 标记为不可卸载 / 不可缓存**(显式标记,而非自动)。
  这是一个 C1↔B2/B3 契约。
- **动态投机长度(C2)必须待在已捕获的 cudagraph 桶内**,否则 eager 回退会抹掉增益。
- **MoE KV 按 DP rank 分区**(`docs/02`)→ 任何跨 rank/全局策略(B7)= 一个 connector +
  共享内容哈希存储,而非单一块管理器。

## 定向研究队列(每个 🔬 模块,在其 phase 之前)
- ✅ **B8 `P(reuse)` 估计器** → `docs/06`(因子化的 流行度×生存;仅 token 类型的 MVP;竞争安全混合)。
- ✅ **C1 MTP** → `docs/05`(C1 = 复用 vLLM 原生 MTP;**C2 = 自建** goodput 感知的 MTP 控制器 —— 那个空位)。
- ✅ **D4/C2 goodput 控制** → `docs/11`(统一 goodput 控制环路;**KV 命中↔投机长度耦合 = 文献空白 = 可守空位**;跨层反馈设计草图 + 开放问题)。
- 🟡 **B5 复用 vs 重算** —— `docs/09` 给了 PCIe↔HBM 成本量级 + A100 重算实践;在线成本模型本体待 P2。
