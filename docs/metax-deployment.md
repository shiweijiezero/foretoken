...
## Containerized k3d validation

A privileged k3d node can run the MetaX device plugin with the required device and library mounts, but those mounts do not by themselves isolate a physical GPU. Docker's NVIDIA `--gpus` option is not a MetaX device-selection interface. Until a MetaX-supported device-selection mechanism or container runtime is configured, request `metax-tech.com/gpu` only with a verified cluster allocation and do not interpret a one-GPU request as a particular host GPU.
