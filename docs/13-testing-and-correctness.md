...
> **状态:纲领(P0 前)。** 项目尚无代码 → 本文先把"小心做测试"拆成两层、立好约定与清单;
> P0 起代码后,代码测试随 [`../tests/`](../tests/) 落地,评测 checklist 随每次开跑强制过一遍。
> 正确性 / 确定性是**一等公民**(`DESIGN §8`、模块 F2),不是事后补的。

## 为什么分两层
"测试"在本项目有两个面,都得严谨:
- **代码正确性测试(L1)** —— 我们的插件(OffloadingManager / CachePolicy / scheduler / connector /
  MTP 控制器)有没有**改变模型输出**、有没有破坏确定性。判据是**无损**:开优化 == 原生 vLLM。
- **评测正确性(L2)** —— 我们报出来的 goodput / 命中率 / 加速**是不是真的**。判据是**可信**:回放
  保真、无陷阱、对标真基线、置信区间分得开。只有真正 real 的评测才知道好坏(`docs/04`/`07`/`10`)。
...
| **确定性** | 相同请求逐字节可复现 | `VLLM_BATCH_INVARIANT=1`(⚠️ A100 上可行性 + 性能损**待核实**,见 `docs/10`)+ 固定种子 | S42 / S43 |
...
### 开跑前 gate(门槛零先过)
- [ ] **回放保真(沉默的杀手)**:离线 `hash_id` 命中率 ≈ 实测原生 APC 命中率;块大小对齐(512)。
- [ ] **PFOO / Belady oracle 就位**,作为 LRU→最优的上界标尺(报"弥合差距 %"才有分母)。
