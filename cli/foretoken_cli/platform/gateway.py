...
from foretoken_cli.platform.helm import (
    Helm,
    PlatformGatewayConfig,
    ReleaseRef,
    envoy_gateway_release,
    managed_envoy_gateway_controller_name,
)
...
class GatewayControllerLifecycle:
    ...
    def resolve_install(
        ...
        preferred_controller = config.controller_name or _ENVOY_GATEWAY_CONTROLLER
    ...
    def resolve_uninstall(
        ...
        helm = self._helm
        kubectl = self._kubectl
        release = envoy_gateway_release(platform.namespace)
        release_exists = helm.release_exists(release)
        managed_release = self._managed_release()
        controller_name = managed_envoy_gateway_controller_name()
        config = (
            helm.platform_gateway_config(platform)
            if platform_exists
            else PlatformGatewayConfig("local", False, "", "", "", "")
