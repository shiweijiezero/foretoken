...
`human` 是用户消息，`gpt` 是参考答案，后续追问默认使用记录中的 `gpt` 答案；添加 `--conversation-history generated` 可改用模型实际生成的回答。需要 system 消息或图片内容时，使用[本地数据指南](conversations_zh.md)中的 OpenAI `messages` 格式。
