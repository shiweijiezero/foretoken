...
func ResolveModelPool(template inferencev1alpha1.NormalizedPoolTemplate, profile RuntimeProfile) (ModelGroupTemplate, error) {
	...
	if profile.Image == "" {
		return ModelGroupTemplate{}, fmt.Errorf("vLLM runtime image is not configured")
	}
	...
	if profile.AcceleratorType == "" || profile.DeviceResourceName == "" || profile.NodeSelectorKey == "" || profile.NodeSelectorValue == "" {
		return ModelGroupTemplate{}, fmt.Errorf("vLLM accelerator profile is incomplete")
	}
	...
	revision, err := revisionFor(resolved)
	if err != nil {
		return ModelGroupTemplate{}, err
	}
...
func revisionFor(template ModelGroupTemplate) (string, error) {
	template.Revision = ""
	payload, err := json.Marshal(template)
	if err != nil {
		return "", fmt.Errorf("encode resolved ModelGroup template: %w", err)
	}
	hash := fnv.New64a()
	hash.Write(payload)
	return "r-" + strconv.FormatUint(hash.Sum64(), 36), nil
}
