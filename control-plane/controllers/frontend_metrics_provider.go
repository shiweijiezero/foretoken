...
func (provider *HTTPPoolMetricsProvider) activeRequests(ctx context.Context, target algorithm.TargetID) (uint64, int64, error) {
	...
	for index := range groups.Items {
		...
		if pool == nil || !modelPoolReady(pool) || !modelGroup.DeletionTimestamp.IsZero() || !modelGroupOwnedByPool(modelGroup, pool) || modelGroup.Spec.Revision != pool.Status.ActiveRevision || !routingGroupReady(modelGroup) {
			continue
		}
