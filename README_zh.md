...
该示例部署一个前端服务、一个 `Qwen/Qwen3-0.6B` 模型副本和一个 10 GiB 运行时缓存 PVC。工作负载请求 1 张 GPU、8 个 CPU 和 52 GiB 内存；还需为平台预留额外容量。资源配置见[单模型示例](examples/quickstart/README_zh.md)，更多部署配置见 [`examples/`](examples/) 目录。
